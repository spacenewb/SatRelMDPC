...
NumLinCycles = 2; % Num. of Lin. cycles to simulate
...
RangeLims = [13.5 10]; % Range Limits for MPC - [max min]
RangeLims = [76 75.8]; % Range Limits for MPC - [max min]

CostA = 10; % Min. Lim Violation Slope
CostB = 10; % Max. Lim Violation Slope
CostRelG = 1e4; % Relative gain (Inputs vs. TrajError)

Tmax = 0.001; % [kN]

Y0_Target = [82106.14, 0.2, deg2rad(98.3), deg2rad(0), deg2rad(270), deg2rad(0)]';
Y0_Chaser = [82106.14, 0.201, deg2rad(98.31), deg2rad(0), deg2rad(270), deg2rad(0)]';
M0 = 12;
...
txt2 = "$CostA: " + CostA + ";~CostB: " + CostB + ";~CostRelG: " + CostRelG + "$";
txt3 = "$Kep_T: (" + strjoin(string(num2str(Y0_Target,'%.4f')),',~') + ")~[km,Rad]$";
...
annotation('textbox', [0.5, 0.885, 0, 0], 'string', txt2, 'FitBoxToText','on',...
    'interpreter','latex', 'HorizontalAlignment','center',...
    'VerticalAlignment','middle','LineStyle','none','Color','red')
annotation('textbox', [0.5, 0.44, 0, 0], 'string', txt3, 'FitBoxToText','on',...
    'interpreter','latex', 'HorizontalAlignment','center',...
    'VerticalAlignment','middle','LineStyle','none','Color','red')
